...
const globalStyles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body { font-family: var(--ff); color: var(--raisin); background: var(--white); }
...
function App() {
  ...
  return (
    <>
      <style>{globalStyles}</style>
      <Header user={user} setPage={setPage} setSelectedProperty={setSelectedProperty} setSelectedService={setSelectedService} />
      <main style={{ marginTop: page === "home" ? "0" : "80px" }}>
        {page === "home" && (
          <>
            <Hero />
            <About />
        ...
        {page === "property" && selectedProperty && (
          <PropertyDetail property={selectedProperty} onBack={() => { setPage("home"); setSelectedProperty(null); }} />
        )}
        {page === "service" && selectedService && (
          <ServiceDetail service={selectedService} onBack={() => { setPage("home"); setSelectedService(null); }} />
        )}
      </main>
    </>
  );
